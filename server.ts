...
/** 始终加载与 server.ts 同目录的 .env，避免从其它 cwd 启动时读不到 Key（表现为 NO_AI_PROVIDER） */
...
async function startServer() {
  ...
  /**
   * 显式 AI_PROVIDER 时：必须配置对应 Key；auto 时按优先级选第一个有 Key 的。
   * 不再在「无 Key」时假装 deepseek（否则会用空 Key 调 API，前端只能退回原文）。
   */
  ...
  /** 上游返回的文本里抠 JSON；避免 JSON.parse("") / 半截文本导致抛错 */
  ...
  /**
   * 合并成可读字符串；保证非空，避免 res.json 因 undefined 字段被前端看成「只有 error」。
   */
  ...
  /** 文本转语音：智谱 GLM-TTS（默认有 GLM Key 时用）或 OpenAI TTS；否则由前端回退浏览器 */
...
请按以下JSON格式返回：
{
  ...
  app.post("/api/ai/image", async (req, res) => {
    ...
    try {
      ...
      if (provider === 'gemini') {
        // Current Gemini image gen usually requires specific models or experimental features
        // Here we use the existing logic but via server environment
        const genAI = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY || '' });
        // Note: For real image gen in Gemini, it often uses different endpoints
        // but since our app was using gemini-2.5-flash-image (experimental/placeholder),
        // we'll keep that path or fallback.
        // For production, we'd use Imagen or DALL-E.
        
        // Let's assume for other providers we might want DALL-E or similar
        const client = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
        ...
        res.json({ url: response.data[0].url });
         ...
         // Fallback or specific provider image gen
